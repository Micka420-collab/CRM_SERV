...
export default function LoanPCs() {
    ...
    const handleEditPC = async (e) => {
        ...
        try {
            ...
            fetchData();
            showAlert('success', 'PC Modifié', `Les informations du PC ${editPC.name} ont été mises à jour.`);
        } catch (error) {
            showAlert('error', 'Erreur', error.response?.data?.error || 'Failed to update PC');
        }
    };
    ...
    const generateSensitiveReport = (allPcs, history, range) => {
        try {
            ...
            doc.setFontSize(11);
            doc.text(`Inventaire et état des PCs de mission - ${new Date().toLocaleDateString('fr-FR')}`, 14, 35);

            // Inventory Table
            autoTable(doc, {
                startY: 55,
                head: [['N° Machine', 'Emprunteur', 'N° Dossier', 'Début Loc', 'Fin Prévue', 'Imputation', 'Observations', 'Dernière Remaster', 'Status']],
                body: sensitivePcs.map(pc => [
                    ...
                    pc.status === 'loaned' ? 'EN PRÊT' : (pc.status === 'available' ? 'DISPONIBLE' : 'HORS SERVICE')
                ]),
            ...
            if (sensitiveHistory.length > 0) {
                ...
                autoTable(doc, {
                    ...
                    body: sensitiveHistory.map(h => [
                        new Date(h.start_date || h.timestamp).toLocaleDateString('fr-FR'),
                        h.action_type === 'loan' ? 'Début Mission' : (h.action_type === 'return' ? 'Fin Mission' : h.action_type),
                        h.pc_name,
                        h.user_name || '-',
            ...
            setShowReportModal(false);
            showAlert('success', 'Succès', 'Rapport missions sensibles téléchargé !');
        } catch (error) {
            console.error('PDF Error:', error);
            showAlert('error', 'Erreur', error.message);
        }
        ...
        <div className="loan-pcs-page">
                                ...
                                <thead>
                                    {activeTab === 'standard' ? (
                                        ...
                                        <tr>
                                            ...
                                            <th>N° de dossier</th>
                                            <th>Date début de Loc</th>
                                            <th>Date de fin de Loc prévu</th>
                                            <th>Retour location</th>
                                            <th>Imputation</th>
                                            <th>Observations</th>
                                            <th>Dernière remasterisation</th>
                                            <th>Status</th>
                                            <th>Actions</th>
                                        </tr>
                                    )}
                                        ...
                                        <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'var(--bg-secondary)', padding: '12px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                                                ...
                                                <div style={{ flex: 1 }}>
                                                    ...
                                                    <div style={{ fontSize: '0.8rem', opacity: 0.7 }}>Activer pour le suivi spécialisé des missions sensibles</div>
                                                </div>
                                    ...
                                    {newPC.is_sensitive && (
                                        <div style={{ border: '1px solid #ef4444', padding: '1.5rem', borderRadius: '12px', background: 'rgba(239, 68, 68, 0.05)', marginBottom: '1.5rem' }}>
                                            ...
                                            <div className="form-row">
                                                ...
                                                <div className="form-group">
                                                    <label>Date de fin de Loc prévu</label>
                                                    <input
                                                        type="date"
            ...
            <ReservationModal
                ...
                onConfirm={() => {
                    ...
                    showAlert('success', 'Planification Réussie', 'Le PC a été planifié avec succès.');
                    loadHistory();
                    fetchData();
                }}
            />
                            ...
                            <div style={{ marginBottom: '2rem' }}>
                                ...
                                <div style={{ 
                                    ...
                                    <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        <label style={{ color: '#94a3b8', fontSize: '0.875rem', fontWeight: 500, marginLeft: '4px' }}>Date de début</label>
                                        <input
                                            type="date"
                        ...
                        <form onSubmit={handleEditPC} className="pro-form">
                            ...
                            <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'var(--bg-secondary)', padding: '12px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontWeight: 600, fontSize: '0.95rem' }}>Mission Sensible</div>
                                        <div style={{ fontSize: '0.8rem', opacity: 0.7 }}>Activer pour le suivi spécialisé</div>
                                    </div>
                                    <label className="toggle-switch">
                                        <input
                                            type="checkbox"
